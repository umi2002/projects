...
{
	QObject::connect(
		&board_, SIGNAL(madeNewBoard()), this, SLOT(displayNewBoard()));
	QObject::connect(&board_,
	                 SIGNAL(madeMove(const QString, int, int)),
	                 this,
	                 SLOT(displayMove(const QString, int, int)));
...
void Interface::displayNewBoard()
{
	...
	for (int i : iter::range(board_.board_.size()))
	{
		...
		for (int j : iter::range(board_.board_[i].size()))
		{
			...
			QObject::connect(box,
			                 SIGNAL(isClicked(int, int)),
			                 &board_,
			                 SLOT(makeMove(int, int)));
...
{
	dynamic_cast<Box*>(boardLayout_->itemAt(x)->layout()->itemAt(y)->widget())
		->setText(s);
}
